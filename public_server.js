...
app.get("/refam/pdf_magazines/:namePdf", (req, res) => {
    var file = fs.createReadStream(__dirname + '/app/public/refam/pdf_magazines/' + req.params.namePdf);
    var stat = fs.statSync(__dirname + '/app/public/refam/pdf_magazines/' + req.params.namePdf);
...
app.use(function (req, res, next) {
    ...
    */
    //if(req.originalUrl.search('autodiscover') > -1){ res.json({error:'URL corrupta'})  ; return false}
    ...
    let domainCorsAdd = '#domain# #domain#:443 #domain#:3001 #domain#:3002 localhost localhost:443 localhost:3001 localhost:3002 10.10.1.103:443'
    if(envData.mode == 'dev')
    ...
    domainCorsAdd = domainCorsAdd
        ...
        let politics = `default-src \'unsafe-inline\' \'unsafe-eval\' \'self\' localhost https://refam.sedena.gob.mx\:\* ;img-src \'self\' data: picsum.photos i.picsum.photos;font-src \'self\' localhost\:\* localhost refam.sedena.gob.mx\:\*;style-src \'self\' localhost\:\* localhost 10.10.1.177:* 10.10.1.177\:\* 10.10.1.103:* 10.10.1.103\:\* 10.10.1.104:* 10.10.1.104\:\* 10.10.1.105:* 10.10.1.105\:\* 10.10.1.106:* 10.10.1.106\:\* 10.10.1.120:* 10.10.1.120\:\* 10.10.1.121:* 10.10.1.121\:\* refam.sedena.gob.mx\:\* \'unsafe-inline\'`
        /*
        console.log( 'default-src \'unsafe-inline\' \'unsafe-eval\' \'self\' '+ domainCorsAdd +' code.jquery.com cdnjs.cloudflare.com maxcdn.bootstrapcdn.com cdn.jsdelivr.net unpkg.com fonts.gstatic.com picsum.photos html2canvas.hertzen.com https://refam.sedena.gob.mx:* ; ' +
...
db.sequelize.sync({ force , alter:false }).then(()=>{
    ...
    if (force == true) {
        db.users.create({enrollment:'daniel',password:'0800'})
        /*
        db.role.create({
            id: 1,
            name: "user"
...
// simple route
app.get("/", (req, res) => {
    //res.json({message: "Welcome to REFAM REST application."})
...
if( envData.mode == 'prod' || 1 == 1){
    try {
        ...
        serverByClusters( { options, app , PORT, protocol })
