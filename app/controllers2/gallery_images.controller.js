...
exports.getAllGalleryImages = (req, res) => {
    ...
    GalleryImage.findAll({where:{status:1}}/*{
...
exports.getGalleryList = (req, res) => {
    GalleryImage.findAll({where:{status:1 /*,magazine:'0' galeria ignorar imagenes de revistas */ }})
        .then( images => {
            let galleryImages = []
            for(let img of images){
                img.name = img.magazine != '' ? img.magazine+'/'+img.name : img.name 
                if(img.status > 0) { 
...
exports.getPageData = (req, res) => {
    ...
    GalleryImage.findAll({status:1})
        .then(galleryImages => {
            let pageGalleryImages = galleryImages.slice(numPerPag * pag - numPerPag, numPerPag * pag)
            resUs.responseData.numOfItems = pageGalleryImages.length
            resUs.responseData.isLastPage = numPerPag * pag >= galleryImages.length;
...
exports.getGalleryImage = (req, res) => {
    ...
    if(fileSystem.existsSync(filePathGallery)){
        ...
        GalleryImage.findOne({where:{name:imageName}})
            .then(result =>{
                if( result != null){
                    let id = result.dataValues.magazine
                    let filePathMagazine = `${__dirname}/../../uploads/gallery/${id}/${imageName}`
...
exports.deleteImage = (req, res) => {
    if(req.query.name != undefined){
        GalleryImage.findOne({where:{name:req.query.name}})
            .then(result =>{
                constants.updateStatus(res, GalleryImage, result.id , 0 , "Eliminada")
                //deleteFromHtml(req.query.name)
            })
    }else{
        if(constants.evalAcces( req , config , role =>{ return (role == 2 || role == 3) } , "requiere nivel 2 o 3 (Editores/Administradores)")){
            constants.updateStatus(res, GalleryImage, req.params.id , 0 , "Eliminada")
        }
    }
}
