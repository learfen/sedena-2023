...
function articleCreateFile(article, next ) {
	...
	next({success:"Archivo Creado" , article})
};

const inspect = async ({where}) => {
	let article = await db.article_magazine.findOne({ where })
	let html = fs.readFileSync(STORAGE_ARTICLE_PATH + article.dataValues.htmlFile + constants.ARTICLE_FILE_EXTENSION).toString()
...
//locals.addEvent('article.saved' , inspect )
//locals.addEvent('article.updated' , inspect )
...
function articleUpdateFile(req, res, file, html ){
	let fileArticle = __dirname + "/../../uploads/magazine_articles/"+file+".html"
...
exports.deleteArticle = (req, res) => {
	if(constants.evalAcces( req , res , config , role =>{ return (role == 1) } , "requiere nivel 1 (Solo escritores)"))
		locals.emit('article.removed' , {id:req.params.id})
}
...
exports.articlePush = ( req, res ) => {
	if(constants.evalAcces( req , res , config , role =>{ return (role == 1) } , "requiere nivel 1 (Solo escritores)"))
		articleCreateFile( 
			{coverPage:req.file.filename,date:"",title:"",html:"",category:req.body.category, magazineId:req.params.magazine} , 
			({success, article}) => {
				locals.emit('article.create' , article)
			}
		)
}
...
exports.articleVisite = ( req, res ) => {
	...
	.then ( article => {
		console.log('visite ' , {article})
		if(article)
			locals.emit('article.update' , { 
				where:{id:req.params.id} , 
				data:{ visite:++article.dataValues.visite } , 
			res })
		else {
			console.log('¿article update > where ', req.params.id)
			res.json({success:'ok'})
		}
	})
}
...
exports.postAddHtmlArticle = (req, res) => {
	...
	if ( !files || files.length == 0) {
		...
		error.httpStatusCode = 400
		console.log("errorcreateSupportIssue= ", error)
		res.status(500).send({message:'Debes seleccionar una imagen.'+ err.message + ""});
	}else{
		...
		let coverPageFileName = files[0].filename

		let articleIdName = articleId.toString() + "-" + articleDate + "-" + articleName
		let articleFileName = articleIdName + constants.ARTICLE_FILE_EXTENSION
		...
		fs.appendFile(filePath, htmlContent, function (err) {
			...
			if (err) {
				console.log("err", err)
				res.status(500).send({message: "Error al crear el archivo HTML. "+ err.message + ""});
			}else{
...
exports.getAllMagazineHtmlArticles = (req, res) => {
	ArticleMagazineModel.findAll({status:1})
		.then(articlesMagazine => {
			res.status(200).send(
				{
					"articlesMagazine": articlesMagazine
				}
			);
		})
		.catch(err => {
...
exports.getHtmlMagazineByIDEdit = (req, res) => {
	...
	MagazineModel.findOne({
		...
		.catch(err => {
			console.log('Error > getHtmlMagazineByIDEdit')
			console.log(err.message);
			res.status(500).send({message: err.message + ""});
		});
...
exports.getTemplateEditorIdArticle = (req, res) => {
	...
	MagazineModel.findOne({
		...
		.catch(err => {
			console.log('Error > getTemplateEditorIdArticle ')
			console.log(err.message);
			res.status(500).send({message: err.message + ""});
		});
...
function responseFileHTML( res ,filePath){
	filePath = path.resolve(filePath)
	if(fs.existsSync(filePath)){
		res.sendFile( filepath )
	}else{
		res.send('<h3>Archivo no encontrado</h3>')
	}
	
}

exports.getMagazineHtmlArticle = (req, res) => {
	responseFileHTML( STORAGE_ARTICLE_PATH + req.body.articleName + constants.ARTICLE_FILE_EXTENSION )
};
...
exports.getMagazineHtmlArticleById = (req, res) => {
	...
	ArticleMagazineModel.findOne(
		...
		responseFileHTML( TORAGE_ARTICLE_PATH + articlesMagazine.htmlFile + constants.ARTICLE_FILE_EXTENSION )

	})
	.catch(err => {
		console.log('Error > getMagazineHtmlArticleById')
		console.log(err.message);
		res.status(500).send({message: err.message + ""});
	});
...
exports.getMagazineHtmlArtById = (req, res) => {
	...
	ArticleMagazineModel.findOne({ where: {id: articleId } }
	).then(articlesMagazine => {
		responseFileHTML( STORAGE_ARTICLE_PATH + articlesMagazine.htmlFile + constants.ARTICLE_FILE_EXTENSION )
	})
		.catch(err => {
			console.log('Error > getMagazineHtmlArtById')
			console.log(err.message);
			res.status(500).send({message: err.message + ""});
		});
...
exports.getMagazineHtmlArtByIdJSON = (req, res) => { 
	...
	ArticleMagazineModel.findOne(
		...
		let page = '<h3>Soy un articulo nuevo</h3>'
		if(!fs.existsSync(filePath)) fs.writeFileSync(filePath , page)
		else page = fs.readFileSync(filePath).toString()
		...
		.catch(err => {
			console.log('Error > getMagazineHtmlArtByIdJSON')
			console.log(err.message);
			res.status(500).send({message: err.message + ""});
		});
...
exports.getMagazineLast = (req, res) => {
	...
	function prepareSend(magazines){
		...
		res.status(200).send( { magazinesFound } )
	}
